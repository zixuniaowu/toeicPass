...
CMD ["bash", "-lc", "PORT=8001 node apps/api/dist/src/main.js & for i in $(seq 1 30); do node -e \"fetch('http://127.0.0.1:8001/api/v1').then(()=>process.exit(0)).catch(()=>process.exit(1))\" 2>/dev/null && break; sleep 1; done; cd apps/web && npx next start -p 7860"]
