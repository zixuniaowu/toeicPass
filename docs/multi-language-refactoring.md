...
- [x] `packages/shared`: 新增 `UiLang`, `NativeLang`, `TargetLang`, `LangConfig` 类型
- [x] `apps/web/lib/i18n.ts`: 创建 `createT()` 函数
- [x] `apps/web/locales/`: 提取核心 COPY 到 zh.json / ja.json
- [x] `apps/web/locales/en.json`: 英文翻译
- [x] `apps/web/hooks/useLangConfig.ts`: 新建语言配置 hook
- [x] 保持 `Locale` 作为 deprecated alias，不破坏现有代码

### Phase 2: 组件迁移 (2-3 周)

- [ ] 所有 `COPY` 组件迁移到 `t()` 函数（当前仍有 11 个组件保留本地 `COPY`，包括 `MockExamView` / `PracticeView` / `VocabView` / `MistakesView` 等）
- [x] TopBar 支持 3 语言切换
- [x] Settings 增加目标语言选择
- [x] ShadowingView 翻译结构改为 `translations: { zh, ja, en }`（应用层数据、TED / 日语 YouTube snapshot 生成产物与对应解析路径已切换到统一结构）
- [ ] Vocab 翻译结构扩展（`translations.definition/example` 结构已接入 API、类型层和主要消费点，当前 JLPT N5/N4/N3 词包也已补到 `zh` / `ja` / `en`；剩余工作是继续补齐更广的种子内容并收缩 runtime fallback）

### Phase 3: 日语学习内容 (3-4 周)

- [x] ConversationAI 增加日语会话场景 + 日语 Gemini prompt
- [ ] 日语词汇包 (JLPT N5-N1 核心词)（已接入首批 N5/N4/N3 词包，并把当前 deck 补成 `zh` / `ja` / `en` 的结构化词义与例句；N2/N1、dashboard、推荐逻辑仍缺）
- [ ] Dashboard 日语版（历史进度、推荐）（当前未发现独立 Dashboard 视图）
- [ ] Writing 日语版（日语作文练习）（已打通 `targetLang` 基础分流，且前后端已支持总评 + rubric + next-step + focusSignals + drillChecklist + revisionPrompt + sentenceFrames 的结构化反馈，并能把建议继续导向 grammar / vocab / shadowing；仍缺更强的 prompt、真正的训练素材闭环与更深的日语专项反馈）

### Phase 4: 高级功能 (可选)

- [ ] 日语文法练习 (JLPT 文法)（已打通最小闭环：`targetLang=ja` 时 grammar tab 会切到 JLPT grammar cards，后端也有独立 `?targetLang=ja` deck；仍缺更完整的题型、等级覆盖与训练深度）
- [ ] 日语阅读理解（已打通最小 Web 链路：`targetLang=ja` 时 reading tab 会切到 `JlptReadingView`，并结合现有 furigana API 展示短篇 JLPT 阅读材料；仍缺更完整的题库、分级题型与答题闭环）
- [ ] JLPT 模拟考试
- [x] 英语母语者专属引导流程

### 8.1 当前剩余 TODO（2026-05-05 审计）

1. 完成剩余 11 个 `COPY` 组件到 `t()` 的迁移，彻底移除分散在组件内的本地文案对象。
2. 在已接入 `translations.definition/example` 的基础上，继续补齐 Vocab 多语内容，并逐步移除 `VocabView` / `FlashCard` 中剩余的 runtime fallback。
3. 在当前确定性 rubric、结构化反馈、改写提示与基础训练跳转的基础上，继续升级写作产品能力，包括更强的 prompt、日语专项反馈，以及与内容层真正联动的训练素材闭环。
4. 补齐剩余日语内容层：把 JLPT 词包补全到 N2/N1，并继续补齐文法、阅读、模拟考试，以及与之配套的 dashboard / 推荐逻辑。
