...
import * as questionBankExpansion10 from "./question-bank-expansion-10.json";
import * as questionBankExpansion11 from "./question-bank-expansion-11.json";
import * as questionBankExpansion12 from "./question-bank-expansion-12.json";
import * as questionBankExpansion13 from "./question-bank-expansion-13.json";
import * as questionBankExpansion14 from "./question-bank-expansion-14.json";
import * as questionBankExpansion15 from "./question-bank-expansion-15.json";
...
export class StoreService {
  ...
  private ensureSeedAds(): void {
    ...
    const adSeeds: Array<{ slot: string; title: string; imageUrl?: string; linkUrl: string; ctaText: string; priority: number }> = [
      ...
      { slot: "interstitial", title: "StudyForge - AI搭載の学習プラットフォーム", imageUrl: "https://huggingface.co/front/assets/huggingface_logo-noborder.svg", linkUrl: "https://huggingface.co/spaces/jackywangsh/studyforge", ctaText: "無料で試す 🤗", priority: 92 },
  ...
  private questionBankDataset(): ImportedQuestionWithSource[] {
    ...
    this.questionBankCache = [
      ...
      ...((questionBankExpansion10 as { questions?: ImportedQuestion[] }).questions ?? []).map((item) => ({
        ...item,
        __source: "bank" as const,
      })),
      ...((questionBankExpansion11 as { questions?: ImportedQuestion[] }).questions ?? []).map((item) => ({
        ...item,
        __source: "bank" as const,
      })),
      ...((questionBankExpansion12 as { questions?: ImportedQuestion[] }).questions ?? []).map((item) => ({
        ...item,
        __source: "bank" as const,
      })),
      ...((questionBankExpansion13 as { questions?: ImportedQuestion[] }).questions ?? []).map((item) => ({
        ...item,
        __source: "bank" as const,
      })),
      ...((questionBankExpansion14 as { questions?: ImportedQuestion[] }).questions ?? []).map((item) => ({
        ...item,
        __source: "bank" as const,
      })),
      ...((questionBankExpansion15 as { questions?: ImportedQuestion[] }).questions ?? []).map((item) => ({
        ...item,
        __source: "bank" as const,
      })),
